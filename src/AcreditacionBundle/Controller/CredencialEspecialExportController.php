 ...
 * Default controller.
 *
 * @Route("/acreditacion/credenciales_esp/imprimir")
 * @Security("has_role('ROLE_ACREDITACION_PRINT')")
 */
class CredencialEspecialExportController extends Controller {
     ...
     * @param CredencialEspecial $personal Personal de los juegos a procesar
     * @param array $param Parámetros de posiconamiento de la acreditación a procesar
     */
    private function certificadoDraw(TCPDF $pdf, CredencialEspecial $personal, $param) {
// set font
...
// define barcode style
        ...
        $pdf->write1DBarcode(strval($personal->getDocumentoNro()), 'C128B', $param['codigo_barra']['x'], $param['codigo_barra']['y'], 100, 16, 0.45, $style, 'M');
        ...
        if (strlen($this->mayuscula($personal->getApellido())) > 15) {
            $pdf->SetFont('helvetica', 'B', 8);
        }
        
          
        $pdf->MultiCell(38, 5, $this->mayuscula($personal->getApellido()), $border = 1, $align = 'C', $fill = true, $ln = 1, $x = $param['datos_pers']['x'], $y = $param['datos_pers']['y'], $reseth = true, $stretch = 0, $ishtml = false, $autopadding = true, $maxh = 0, $valign = 'M', $fitcell = true
        );
        //set Nombre
        if (strlen($this->mayuscula($this->mayuscula($personal->getNombre()))) > 15) {
            $pdf->SetFont('helvetica', 'B', 8);
        }
        
        $pdf->MultiCell(38, 5, $this->mayuscula($personal->getNombre()), $border = 1, $align = 'C', $fill = true, $ln = 1, $x = $param['datos_pers']['x'], $y = $param['datos_pers']['y'] + 5, $reseth = true, $stretch = 0, $ishtml = false, $autopadding = true, $maxh = 0, $valign = 'M', $fitcell = true
        );
        //set DNI tipo
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->MultiCell(38, 5, $this->mayuscula($personal->getDocumentoTipo()->getNombre()) . ' ' . $personal->getDocumentoNro(), $border = 1, $align = 'C', $fill = true, $ln = 1, $x = $param['datos_pers']['x'], $y = $param['datos_pers']['y'] + 10, $reseth = true, $stretch = 0, $ishtml = false, $autopadding = true, $maxh = 0, $valign = 'M', $fitcell = true
        );
    }
        ...
        for ($index = 1; $index <= $cantPaginas; $index++) {
            ...
            for ($indexj = 1; $indexj <= 4; $indexj++) {
                if ((($index - 1) * 4) + $indexj <= count($param)) {
                    $entity = $em->getRepository('AcreditacionBundle:CredencialEspecial')->find($param[(($index - 1) * 4) + $indexj - 1]);
                    if (!($entity)) {
                        return 'error';
                    }
...
//Close and output PDF document
        $pdf->Output('AcreditacionesEspeciales.pdf', 'I');
    }
